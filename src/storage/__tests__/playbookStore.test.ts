...
describe('playbook store', () => {
  ...
  it('resolves sequence items in order preserving reps and falls back to all plays when empty', () => { const other = { ...play, id: 'p2' }; expect(resolveSequence([play, other], [{ playId: 'p2' }, { playId: 'p1' }])).toEqual({ playlist: [other, play], reps: [1, 1] }); expect(resolveSequence([play, other], [{ playId: 'missing' }, { playId: 'p2' }])).toEqual({ playlist: [other], reps: [1] }); expect(resolveSequence([play, other], [{ playId: 'p2', reps: 3 }])).toEqual({ playlist: [other], reps: [3] }); expect(resolveSequence([play, other], [{ playId: 'p2' }, { playId: 'missing', reps: 4 }])).toEqual({ playlist: [other], reps: [1] }); expect(resolveSequence([play, other], [])).toEqual({ playlist: [play, other], reps: [1, 1] }); });
